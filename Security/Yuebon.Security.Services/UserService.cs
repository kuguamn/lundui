...
public class UserService : BaseService<User, UserOutputDto>, IUserService
{
    ...
    public  async Task<PageResult<UserOutputDto>> FindWithPagerSearchAsync(SearchUserModel search)
    {
        ...
        if (search.Organizeid != 0)
        {
           orgIds =await _organizeService.GetChildIdListWithSelfById((long)search.Organizeid);
        }
        ...
        exp.AndIF(!string.IsNullOrEmpty(search.Station), it => it.Station == search.Station);
        ...
        exp.AndIF(orgIds.Count!= 0, it =>orgIds.Contains((long)it.Organizeid));
        ...
        foreach (UserOutputDto item in resultList)
        {
            if (item.CreateOrgId >0)
            {
                ...
                item.RoleName = await _userRoleService.GetUserRoleNames(item.Id);
            listResult.Add(item);
        }
    ...
    /// <returns></returns>
    public async Task<bool> CheckMacExists(string mac)
    {
        string sql = "select * from sys_mac where mac=@mac";
        var result = await _userRepository.Db.Ado.SqlQueryAsync<dynamic>(sql, new { mac });
        return result.Any();
    }
