        ...
        try
        {
            ...
            foreach (var item in assemblyList)
            {
                ...
                else
                {
                    ...
                    ConsoleHelper.WriteInfoLine($"不处理{assembly.GetName().Name}.dll 中业务服务Service、仓储Repositories的接口和实现注入", ConsoleColor.Red);
                }
            }
        }
        catch (Exception ex)
        {
            ConsoleHelper.WriteInfoLine($"serviceType:{ex.Message}", ConsoleColor.Red);
        }
